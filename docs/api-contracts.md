...
| DELETE | `/properties/{id}` | Delete property (DRAFT only) |
...
- `display_status`: Human-readable status label
- `allowed_actions`: Array of legal actions
- `visibility`: Flags for conditional UI elements
...
| DELETE | `/agent-assignments/{id}` | Cancel agent request |
...
**State Transition:**
```
Entry: properties.status = DRAFT
Action: POST /agent-assignments
Exit: properties.status = PENDING_ASSIGNMENT
      agent_assignments.status = REQUESTED

Agent Accepts: properties.status → ASSIGNED
Agent Declines: properties.status → DRAFT
```
